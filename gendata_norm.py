from features_from_phs import gen_features_norm, is_simulation_file
...
@click.option('-l', '--lower', default=30, type=int, help='lower border of time slices to use')
@click.option('-u', '--upper', default=70, type=int, help='upper border of time slices to use')
def main(output_file, input_file, eps, n_jobs, lower, upper):
    ...
    with Pool(n_jobs) as pool:
        results = [pool.apply_async(gen_features_norm, kwds={'data_file': f, 'lower': lower, 'upper': upper}) for f in input_file]
